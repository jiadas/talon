...
func (s *LogStore) GetLogsBytes(shardID int, cursor, endCursor string,
	...
	r, err := request(s.project, "GET", uri, h, nil)
	if err != nil {
		err = fmt.Errorf("failed to request:%v", err)
		return
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to Atoi:%v", err)
		return
	}
	...
	if err != nil {
		err = fmt.Errorf("failed to Uncompress:%v", err)
		return
	}
...
func (s *LogStore) PullLogs(shardID int, cursor, endCursor string,
	...
	if err != nil {
		return nil, "", fmt.Errorf("failed to LogsBytesDecode:%v", err)
	}
